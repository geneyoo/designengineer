...
The contract was extracted from a production multi-agent repository with
worktree, simulator-pool, merge-queue, hook, and admission tests. The source
repository is not required to adopt it; every necessary invariant is stated
here and in [`development-flow.md`](development-flow.md).
  ...
  writer-isolation:
    ...
    guards:
      ...
      - surface: editor-tool
        ...
        command: ./scripts/agent-worktree-guard.sh
        escape: AGENT_ALLOW_UNMANAGED_EDITS
        fails: open
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$CLAUDE_PROJECT_DIR/scripts/agent-worktree-guard.sh\"",
            "timeout": 10,
...
`AGENT_CHECK_MODE=verification git push` opts a single push into the slow lane.
...
- **The PR verb is a script, not a provider command repeated in prompts.** A
  checked-in wrapper centralizes authentication, body requirements, draft
  state, and temporary provider workarounds. One script changes instead of
  every agent learning a new exception.
...
scripts/agent-worktree-guard-test.sh   allow, deny, fail-open, escape hatch
